...
void dump_rdmult_value(MACROBLOCK *const x, int mi_row, int mi_col, BLOCK_SIZE bsize, RUN_TYPE dry_run, char *function_name) {
  FILE *fptr;
  fptr = fopen("/home/lambda-ipi/Documents/metric_in_aom/rdmultnnn.txt", "a");
  if (fptr == NULL) {
    printf("Error creating the pointer to the dump file for rdmult");
    exit(1);
  }
  //fprintf(fptr, "%f,", (double) scaling_factors[index]);
  fprintf(fptr, "%d,%d,%d,%d,%d,%s", x->rdmult, mi_row, mi_col, bsize, dry_run, function_name);
  fprintf(fptr,"\n");
  fclose(fptr);
}
...
static void encode_superblock(const AV1_COMP *const cpi, TileDataEnc *tile_data,
  ...
  if (dry_run == 0 && x->rdmult != 3334897){
    int r = 0;
  }
  //dump_rdmult_value(x, mi_row, mi_col, bsize, dry_run, "encode_superblock");
...
static void setup_block_rdmult(const AV1_COMP *const cpi, MACROBLOCK *const x,
  ...
  if (aq_mode != NO_AQ) {
    ...
    if (aq_mode == VARIANCE_AQ) {
      ...
      //dump_rdmult_value(x, mi_row, mi_col, bsize, 10, "setup_block_rdmult_aq_variance"); // mbmi->segment_id
#if CONFIG_TUNE_RDMULT_DUMMY
    } else if (aq_mode == WPSNR_AQ) {
      if (cpi->vaq_refresh) {
        //const int energy = av1_block_wpsnr(cpi, bsize, mi_row, mi_col);
        //mbmi->segment_id = energy;
        const double energy = av1_block_logwk(cpi, bsize, mi_row, mi_col); // should we use a default mb_energy as in VARIANCE_AQ
        // not sure we really need since we are already using a precomputed map on 16x16 blocks

        assert(energy >= cpi->min_logwk && energy <= cpi->max_logwk);
        int i = (int)((energy - cpi->min_logwk) / cpi->wk_bin_width);// min_logwk + i x bin_w = ith_bound
        if (i > 7) i = 7;
        //assert(i < 8); //maximum 8 segmentation maps
        mbmi->segment_id = i;
      }
#if CONFIG_TUNE_RDM_SEP_AQ == 2
      x->rdmult = x->rdmult;
#elif CONFIG_TUNE_RDM_SEP_AQ == 1
      av1_set_rdmult_dummy(cpi, &x->errorperbit, bsize, mi_row, mi_col,
                           &x->rdmult);
#else
      x->rdmult = set_segment_rdmult(cpi, x, mbmi->segment_id);
#endif
      //dump_rdmult_value(x, mi_row, mi_col, bsize, 10, "setup_block_rdmult_aq_wpsnr"); // mbmi->segment_id
#endif
#if CONFIG_SALIENCY_MODE
    } else if (aq_mode == SALIENCY_AQ) {
      if (cpi->vaq_refresh) {
        const int energy = av1_block_saliency(cpi, bsize, mi_row, mi_col);
        mbmi->segment_id = energy;
      }
      x->rdmult = set_segment_rdmult(cpi, x, mbmi->segment_id);
#endif
    ...
    } else if (aq_mode == ALPHA_BETA_AQ) {
      if (cpi->vaq_refresh) {
        const int seg_id = av1_block_alpha_beta_aq(cpi, bsize, mi_row, mi_col);
        if (seg_id == 1) {
          //gg
          int hhhj = 1;
        }
        mbmi->segment_id = seg_id;
      }
      x->rdmult = set_segment_rdmult(cpi, x, mbmi->segment_id);
    }
# if CONFIG_A_MODE
    else if (aq_mode == ALPHA_AQ) {
      if (cpi->vaq_refresh) {
        const int seg_id = av1_block_alpha_aq(cpi, bsize, mi_row, mi_col);
        if (seg_id == 1) {
          //gg
          int hhhj = 1;
        }
  ...
  if (cpi->oxcf.tune_cfg.tuning == AOM_TUNE_SSIM) {
    ...
    av1_set_rdmult_dummy(cpi, &x->errorperbit, bsize, mi_row, mi_col,
                         &x->rdmult);
    //dump_rdmult_value(x, mi_row, mi_col, bsize, "setup_block_rdmult_tune_wpsnr");
#else
#if CONFIG_SALIENCY_MODE
    av1_set_rdmult_saliency(cpi, &x->errorperbit, bsize, mi_row, mi_col,
                        &x->rdmult);
#else
#if CONFIG_AB_MODE
    av1_set_rdmult_abmode(cpi, &x->errorperbit, bsize, mi_row, mi_col,
                          &x->rdmult, &x->factor_a, &x->factor_b, &x->spy_tracker);
#else
#if CONFIG_A_MODE
    av1_set_a_mode_rdmult(cpi, &x->errorperbit, bsize, mi_row, mi_col,
                          &x->rdmult);
#else
    av1_set_ssim_rdmult(cpi, &x->errorperbit, bsize, mi_row, mi_col,
                        &x->rdmult);
    //dump_rdmult_value(x, mi_row, mi_col, bsize, 10, "setup_block_rdmult_tune_ssim"); // mbmi->segment_id
#endif // AB_MODE
#endif // SALIENCY
#endif // CONFIG_TUNE_RDMULT_DUMMY
#endif
  }
...
#if CONFIG_AB_MODE
  av1_rd_cost_update(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, &best_rd);
#else
...
#if CONFIG_AB_MODE
  av1_rd_cost_update(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, &best_rdcost);
#else
...
#if CONFIG_AB_MODE
  av1_rd_stats_subtraction(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, &best_rdcost, sum_rdc, &rdcost_remaining);
#else
...
#if CONFIG_AB_MODE
    av1_rd_cost_update(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, sum_rdc);
#else
...
#if CONFIG_AB_MODE
  av1_rd_stats_subtraction(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, best_rdc, &part_search_state->sum_rdc,
                           &best_remain_rdcost);
#else
...
#if CONFIG_AB_MODE
  av1_rd_cost_update(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, &part_search_state->this_rdc);
#else
...
#if CONFIG_AB_MODE
    av1_rd_cost_update(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, &part_search_state->sum_rdc);
#else
...
#if CONFIG_AB_MODE
    av1_rd_cost_update(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, &partition_rdcost);
    av1_rd_stats_subtraction(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, best_rdc, &partition_rdcost,
                             best_remain_rdcost);
#else
...
#if CONFIG_AB_MODE
  av1_rd_cost_update(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, this_rdc);
#else
...
static void split_partition_search(
  ...
  for (idx = 0; idx < SUB_PARTITIONS_SPLIT && sum_rdc.rdcost < best_rdc->rdcost;
    ...
    RD_STATS best_remain_rdcost;
#if CONFIG_AB_MODE
    av1_rd_stats_subtraction(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, best_rdc, &sum_rdc,
                             &best_remain_rdcost);
#else
...
static void write_partition_tree(AV1_COMP *const cpi,
  ...
  //         cpi->sb_counter, 0);
  snprintf(filename, sizeof(filename), "%s/partition_tree_sb%d_c%d", "/Users/andreas/Documents",
           cpi->sb_counter, 0);
  FILE *pfile = fopen(filename, "w");
  fprintf(pfile, "\n%d\n", bsize);
  ...
  while (num_nodes > 0) {
    const PC_TREE *node = tree_node_queue[q_idx];
    if (node->partitioning == PARTITION_SPLIT) {
      fprintf(pfile, "%d,%d\n", node->partitioning, node->block_size);
    } else {
      switch (node->partitioning) {
        case PARTITION_NONE:
          fprintf(pfile, "%d,NONE,%d,", node->partitioning, node->block_size);
          fprintf(pfile, "%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,%lld,%lld\n", node->none->rd_stats.rate, node->none->rd_stats.dist, node->none->rd_stats.rdcost);
          break;
        case PARTITION_HORZ:
          fprintf(pfile, "%d,HORZ,%d,", node->partitioning,node->block_size);
          //fprintf(pfile, "%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,%lld,%lld", node->horizontal[0]->rd_stats.rate, node->horizontal[0]->rd_stats.dist, node->horizontal[0]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld\n", node->horizontal[1]->rd_stats.rate, node->horizontal[1]->rd_stats.dist, node->horizontal[1]->rd_stats.rdcost);
          break;
        case PARTITION_VERT:
          fprintf(pfile, "%d,VERT,%d,", node->partitioning, node->block_size);
          fprintf(pfile, "###%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,", node->none->mic.current_qindex);
          fprintf(pfile, "%d,", node->vertical[0]->mic.segment_id);
          fprintf(pfile, "%d,###", node->vertical[0]->mic.current_qindex);
          fprintf(pfile, "%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,%lld,%lld", node->vertical[0]->rd_stats.rate, node->vertical[0]->rd_stats.dist, node->vertical[0]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld\n", node->vertical[1]->rd_stats.rate, node->vertical[1]->rd_stats.dist, node->vertical[1]->rd_stats.rdcost);
          break;
        case PARTITION_HORZ_A:
          fprintf(pfile, "%d,HORZ_A,%d,", node->partitioning, node->block_size);
          fprintf(pfile, "%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,%lld,%lld", node->horizontala[0]->rd_stats.rate, node->horizontala[0]->rd_stats.dist, node->horizontala[0]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld", node->horizontala[1]->rd_stats.rate, node->horizontala[1]->rd_stats.dist, node->horizontala[1]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld\n", node->horizontala[2]->rd_stats.rate, node->horizontala[2]->rd_stats.dist, node->horizontala[2]->rd_stats.rdcost);
          break;
        case PARTITION_HORZ_B:
          fprintf(pfile, "%d,HORZ_B,%d,", node->partitioning, node->block_size);
          fprintf(pfile, "%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,%lld,%lld", node->horizontalb[0]->rd_stats.rate, node->horizontalb[0]->rd_stats.dist, node->horizontalb[0]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld", node->horizontalb[1]->rd_stats.rate, node->horizontalb[1]->rd_stats.dist, node->horizontalb[1]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld\n", node->horizontalb[2]->rd_stats.rate, node->horizontalb[2]->rd_stats.dist, node->horizontalb[2]->rd_stats.rdcost);
          break;
        case PARTITION_VERT_A:
          fprintf(pfile, "%d,VERT_A,%d,", node->partitioning, node->block_size);
          fprintf(pfile, "%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,%lld,%lld", node->verticala[0]->rd_stats.rate, node->verticala[0]->rd_stats.dist, node->verticala[0]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld", node->verticala[1]->rd_stats.rate, node->verticala[1]->rd_stats.dist, node->verticala[1]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld\n", node->verticala[2]->rd_stats.rate, node->verticala[2]->rd_stats.dist, node->verticala[2]->rd_stats.rdcost);
          break;
        case PARTITION_VERT_B:
          fprintf(pfile, "%d,VERT_B,%d,", node->partitioning, node->block_size);
          fprintf(pfile, "%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,%lld,%lld", node->verticalb[0]->rd_stats.rate, node->verticalb[0]->rd_stats.dist, node->verticalb[0]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld", node->verticalb[1]->rd_stats.rate, node->verticalb[1]->rd_stats.dist, node->verticalb[1]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld\n", node->verticalb[2]->rd_stats.rate, node->verticalb[2]->rd_stats.dist, node->verticalb[2]->rd_stats.rdcost);
          break;
        case PARTITION_HORZ_4:
          fprintf(pfile, "%d,HORZ_4,%d,", node->partitioning, node->block_size);
          fprintf(pfile, "%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,%lld,%lld", node->horizontal4[0]->rd_stats.rate, node->horizontal4[0]->rd_stats.dist, node->horizontal4[0]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld", node->horizontal4[1]->rd_stats.rate, node->horizontal4[1]->rd_stats.dist, node->horizontal4[1]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld", node->horizontal4[2]->rd_stats.rate, node->horizontal4[2]->rd_stats.dist, node->horizontal4[2]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld\n", node->horizontal4[3]->rd_stats.rate, node->horizontal4[3]->rd_stats.dist, node->horizontal4[3]->rd_stats.rdcost);
          break;
        case PARTITION_VERT_4:
          fprintf(pfile, "%d,VERT_4,%d,", node->partitioning, node->block_size);
          fprintf(pfile, "###%d,", node->none->mic.segment_id);
          fprintf(pfile, "%d,", node->none->mic.current_qindex);
          fprintf(pfile, "%d,", node->vertical4[0]->mic.segment_id);
          fprintf(pfile, "%d,###", node->vertical4[0]->mic.current_qindex);
          fprintf(pfile, "%d,%lld,%lld", node->vertical4[0]->rd_stats.rate, node->vertical4[0]->rd_stats.dist, node->vertical4[0]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld", node->vertical4[1]->rd_stats.rate, node->vertical4[1]->rd_stats.dist, node->vertical4[1]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld", node->vertical4[2]->rd_stats.rate, node->vertical4[2]->rd_stats.dist, node->vertical4[2]->rd_stats.rdcost);
          fprintf(pfile, "%d,%lld,%lld\n", node->vertical4[3]->rd_stats.rate, node->vertical4[3]->rd_stats.dist, node->vertical4[3]->rd_stats.rdcost);
          break;
        default: assert(0);
      }
    }
...
#if CONFIG_AB_MODE
        av1_rd_cost_update(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, this_rdcost);
#else
...
#if CONFIG_AB_MODE
  av1_rd_cost_update(x->rdmult, x->factor_a, x->factor_b, x->spy_tracker, x->e_mbd.width * x->e_mbd.height, &best_rdc);
#else
...
BEGIN_PARTITION_SEARCH:
  ...
  */
  ...
  //dump_rdmult_value(x, mi_row, mi_col, bsize, "av1_rd_pick_partition");
