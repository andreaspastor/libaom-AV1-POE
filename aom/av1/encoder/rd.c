...
int av1_compute_rd_mult_based_on_qindex(aom_bit_depth_t bit_depth,
  ...
  int rdmult = q * q;
  if (update_type == KF_UPDATE) { // andreas: update rdmult for key frame type of frame
    double def_rd_q_mult = def_kf_rd_multiplier(qindex);
    rdmult = (int)((double)rdmult * def_rd_q_mult);
  } else if ((update_type == GF_UPDATE) || (update_type == ARF_UPDATE)) { // andreas: update rdmult for Golden frame type of frame
