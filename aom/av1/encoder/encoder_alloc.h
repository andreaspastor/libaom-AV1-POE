...
#if CONFIG_A_MODE
  aom_free(cpi->factors_a);
  cpi->factors_a = NULL;
#endif
  aom_free(cpi->ab_qps);
  aom_free(cpi->ab_deltaq);
  cpi->ab_qps = NULL;
  cpi->ab_deltaq = NULL;
#if CONFIG_AB_MODE
  aom_free(cpi->ab_factors_a);
  aom_free(cpi->ab_factors_b);
  cpi->ab_factors_a = NULL;
  cpi->ab_factors_b = NULL;
#endif
#if CONFIG_SALIENCY_MODE
  aom_free(cpi->saliency_factors);
  cpi->saliency_factors = NULL;
#endif

#if CONFIG_TUNE_RDMULT_DUMMY
  aom_free(cpi->wk_rdmult_scaling_factors);
  cpi->wk_rdmult_scaling_factors = NULL;
  aom_free(cpi->wkx_rdmult_scaling_factors);
  cpi->wkx_rdmult_scaling_factors = NULL;

  aom_free(cpi->wk_high_pass_filtered);
  cpi->wk_high_pass_filtered = NULL;
#endif
