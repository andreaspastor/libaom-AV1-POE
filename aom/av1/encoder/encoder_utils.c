...
static int frame_id = 1; // to keep track of the current frame id to dump data in correct file
...
void av1_set_mb_ssim_rdmult_scaling(AV1_COMP *cpi) {
...
#if CONFIG_TUNE_SSIM_DOUBLE
            var += av1_get_sby_perpixel_variance_double(cpi, &buf, BLOCK_8X8);
#else
  ...
  dump_scaling_maps(cpi->ssim_rdmult_scaling_factors, num_rows, num_cols, (char *) cpi->oxcf.tune_cfg.dump_folder, "ssim_map.txt");
  frame_id++; // end of compute for this frame id
}

void dump_scaling_maps(double * scaling_factors, int num_rows, int num_cols, char *folder_name, char *filename) {
  FILE *fptr;

  /*char buffer[11];
  sprintf(buffer, "frame%05d", frame_id);

  char* name_with_extension;
  name_with_extension = malloc(strlen(folder_name) + strlen(filename) + strlen(buffer));
  strcpy(name_with_extension, folder_name);
  strcat(name_with_extension, buffer);
  strcat(name_with_extension, filename);*/

  char name_with_extension[1024];
  snprintf(name_with_extension, sizeof(name_with_extension), "%sframe%05d%s", folder_name, frame_id, filename);
  fptr = fopen(name_with_extension, "w");

  if (fptr == NULL) {
    printf("Error creating the pointer to the dump file");
...
void av1_set_saliency(AV1_COMP *cpi, char *folder_name, char *filename) {
  FILE *fptr;
  ...
  char* name_with_extension;
  name_with_extension = malloc(strlen(folder_name) + strlen(filename) + strlen(buffer) + 12);
  strcpy(name_with_extension, folder_name);
  strcat(name_with_extension, "saliency_in/");
  strcat(name_with_extension, buffer);
  strcat(name_with_extension, filename);

  // "/Users/andreas/Documents/repos/TranSalNet/saliency_info.txt"
  // /Users/andreas/Documents/clion_encode/data_ssim/frame00005saliency.txt
  fptr = fopen(name_with_extension, "r");
  if (fptr == NULL) {
    printf(name_with_extension);
    printf(" Error! Could not open the file..");
    exit(1);
  }

  //const int nb_max_character_per_row = cpi->frame_info.frame_width * 4 + 1;
  char line[3840 * 4 + 1]; // one comma and 3digits max
  int value, start_ind, index;
  //int row_index = 0;
  //int col_index = 0;
  //int height = cpi->frame_info.frame_height;
  int width = cpi->frame_info.frame_width;
  int px = 0;
  int py = 0;
  while (fgets(line, 3840 * 4 + 1, fptr) != NULL)
  {
    //printf("The line is: %s\n", line);
    start_ind = 0;
    px = 0;
    for (int i = 0; i  < 3840 * 4 + 1; i++) {
      if (line[i] == ',') { // find the spacer of the number in the file
        value = convert_slice(line, start_ind, i);
        index = px + py * width;
        cpi->saliency_factors[index] = ((double) value) / 255;
        //cpi->saliency_factors[index] = ((double) value + 1) / 255 * 10;//2
        //cpi->saliency_factors[index] = 2 / ((double) value + 1);
        //cpi->saliency_factors[index] = (256 - ((double) value)) / 255 + 1;
        //cpi->saliency_factors[index] = (256 - ((double) value)) / 255 * 2;
        start_ind = i + 1;
        px++;
  ...
  frame_id++; // increase the id for the next frame
}
#endif // SALIENCY

#if CONFIG_A_MODE
void av1_set_amode(AV1_COMP *cpi, char *folder_name, char *filename) {
  FILE *fptr;
  ...
  char* name_with_extension;
  name_with_extension = malloc(strlen(folder_name) + strlen(filename) + strlen(buffer) + 12 + 7);
  strcpy(name_with_extension, folder_name);
  //strcat(name_with_extension, "alpha_beta_in/");
  //strcat(name_with_extension, "svr_alpha/");
  strcat(name_with_extension, buffer);
  strcat(name_with_extension, filename);
  ...
  int num_doubles = 0;
  //Read the doubles from the file into the array
  double temp, wa;
  while (fscanf(fptr, "%lf,", &temp) == 1) {
    //wa = exp(- temp) - 1e-5;
    //wa = pow((apic / wa), 0.5);

    if (num_doubles > num_cols * num_rows) {
      cpi->factors_a[num_doubles] = 1; // warning something wrong out of range
    }

    if (cpi->oxcf.q_cfg.aq_mode == ALPHA_AQ) {// this is for AQmode 7
      wa = temp;
    } else {
      wa = apic / temp;

      wa = temp; // from the new version to encode with tune==ssim alphas
    }
    cpi->factors_a[num_doubles++] = wa;

    if (wa < cpi->min_factor_a) {
      cpi->min_factor_a = wa;
    }
    if (wa > cpi->max_factor_a) {
      cpi->max_factor_a = wa;
    }
  }
  printf("The array alpha contains %d doubles expecting %d.\n", num_doubles, num_cols * num_rows);
  printf("last value %f %f %f.\n", wa, cpi->min_factor_a, cpi->max_factor_a);
  fclose(fptr);
  frame_id++;
}
#endif

void av1_set_alpha_beta_aq(AV1_COMP *cpi, char *folder_name, char *filename) {
  const int base_qindex = cpi->oxcf.rc_cfg.cq_level / 4;
  ...
  sprintf(buffer, "frame%05d", frame_id);
  char buffer_qp[7];
  sprintf(buffer_qp, "_qp_%02d", base_qindex);
  ...
  strcat(name_with_extension, "alpha_beta_in/");
  strcat(name_with_extension, buffer);
  strcat(name_with_extension, buffer_qp);
  strcat(name_with_extension, filename);
  ...
  while (fscanf(fptr, "%lf,", &temp) == 1) {
    if (num_doubles < 8) {
      // the 8 first values in the file are for the segmentation map
      cpi->ab_deltaq[num_doubles++] = temp;
    } else {
      cpi->ab_qps[num_doubles - 8] = temp;
      num_doubles++;
    }
  }
  printf("The array alpha contains %d qps.\n", num_doubles - 8);
  fclose(fptr);
  // Do not increase yet the frame id since we need to load also the beta values
}

#if CONFIG_AB_MODE
void av1_set_abmode_alpha(AV1_COMP *cpi, char *folder_name, char *filename) {
  FILE *fptr;
  ...
  double temp;
  while (fscanf(fptr, "%lf,", &temp) == 1) {
    cpi->ab_factors_a[num_doubles++] = exp(- temp) - 1e-5;
  }
  printf("The array alpha contains %d doubles.\n", num_doubles);
  fclose(fptr);
  // Do not increase yet the frame id since we need to load also the beta values
}

void av1_set_abmode_beta(AV1_COMP *cpi, char *folder_name, char *filename) {
  FILE *fptr;
  ...
  double temp;
  while (fscanf(fptr, "%lf,", &temp) == 1) {
    cpi->ab_factors_b[num_doubles++] = exp(- temp) - 1e-5;
  }
  printf("The array beta contains %d doubles.\n", num_doubles);
  fclose(fptr);
  frame_id++; // increase the id for the next frame
}
#endif // AB_MODE
...
void av1_set_mb_wk_rdmult_scaling(AV1_COMP *cpi) {
  const CommonModeInfoParams *const mi_params = &cpi->common.mi_params;
  //ThreadData *td = &cpi->td;
  //MACROBLOCK *x = &td->mb;
  //MACROBLOCKD *xd = &x->e_mbd;
  ...
  double filter_value;
  for (int pos_y = 1; pos_y < height - 1; pos_y++) {
    for (int pos_x = 1; pos_x < width - 1; pos_x++) {
      //if (pos_y == 718) {
      //  int e;
      //}

      //filter_value = 8 * y_buffer[pos_x + y_stride * pos_y] - 1 * y_buffer[pos_x + 1 + y_stride * pos_y] - 1 * y_buffer[pos_x - 1 + y_stride * pos_y];
      //filter_value -= y_buffer[pos_x - 1 + y_stride * (pos_y - 1)] + 1 * y_buffer[pos_x + y_stride * (pos_y - 1)] + y_buffer[pos_x + 1 + y_stride * (pos_y - 1)];
      //filter_value -= y_buffer[pos_x - 1 + y_stride * (pos_y + 1)] + 1 * y_buffer[pos_x + y_stride * (pos_y + 1)] + y_buffer[pos_x + 1 + y_stride * (pos_y + 1)];

      //filter_value = 4 * y_buffer[pos_x + y_stride * pos_y] - y_buffer[pos_x + 1 + y_stride * pos_y] - y_buffer[pos_x - 1 + y_stride * pos_y];
      //filter_value -= y_buffer[pos_x + y_stride * (pos_y - 1)];
      //filter_value -= y_buffer[pos_x + y_stride * (pos_y + 1)];

      filter_value = 12 * y_buffer[pos_x + y_stride * pos_y] - 2 * y_buffer[pos_x + 1 + y_stride * pos_y] - 2 * y_buffer[pos_x - 1 + y_stride * pos_y];
      filter_value -= y_buffer[pos_x - 1 + y_stride * (pos_y - 1)] + 2 * y_buffer[pos_x + y_stride * (pos_y - 1)] + y_buffer[pos_x + 1 + y_stride * (pos_y - 1)];
      filter_value -= y_buffer[pos_x - 1 + y_stride * (pos_y + 1)] + 2 * y_buffer[pos_x + y_stride * (pos_y + 1)] + y_buffer[pos_x + 1 + y_stride * (pos_y + 1)];
      //filter_value /= 4;

      index = (pos_x - 1) + (pos_y - 1) * (width - 2); // width - 2 since the map is missing 1px on each side due to the size of the filter
      cpi->wk_high_pass_filtered[index] = filter_value;
    }
  }
  dump_scaling_maps(cpi->wk_high_pass_filtered, height - 2, width - 2, (char *) cpi->oxcf.tune_cfg.dump_folder, "wk_filtered_map.txt");

  int BD = 8;
  double apic = pow(2, BD); //2 **  (BD + 1) # one for wpsnr and one for xpsnr
  apic *= pow((3840 * 2160) / (width * height), 0.5);
  //apic = 2048;
  double amin2 = pow(pow(2, BD - 6), 2);
  ...
  double energy = 0;
  double log_sum = 0;
  double min_logwk = 1000;
  double max_logwk = -1000;
  double logwk;
  ...
  for (int row = 0; row < num_rows; ++row) {
    for (int col = 0; col < num_cols; ++col) {
      ...
      for (int py = 0; py < 16; py++) {
        if (row * 16 + py >= height - 2) continue ;
        for (int px = 0; px < 16; px++) {
          if (col * 16 + px >= width - 2) continue ;
          index2 = (col * 16 + px) + (row * 16 + py) * (width - 2);
          energy += fabs(cpi->wk_high_pass_filtered[index2]);
          nb_pixels++;
        }
      }
      //energy /= 16 * 16;
      ...
      if (energy > amin2) {
        wk = pow(apic / energy, 0.5);
      } else {
        wk = pow(apic / amin2, 0.5);
      }

      // Do we need to perform some fitting there like in av1_set_mb_ssim_rdmult_scaling ?
#if CONFIG_WK_WKX_INV_MODE
      cpi->wk_rdmult_scaling_factors[index] = 1/wk; // /wk; // 10/wk, 1;
      log_sum += log(1/wk);
#else
      cpi->wk_rdmult_scaling_factors[index] = wk;
      log_sum += log(wk);
#endif
      logwk = 3 * log(1/wk)/log(2);
      if (logwk > max_logwk) {
        max_logwk = logwk;
      } else if (logwk < min_logwk) {
        min_logwk = logwk;
      }
    }
  }
  dump_scaling_maps(cpi->wk_rdmult_scaling_factors, num_rows, num_cols, (char *) cpi->oxcf.tune_cfg.dump_folder, "wk_map.txt");

  double wk_range = max_logwk - min_logwk;
  double wk_bin_width = wk_range / 8;
  cpi->wk_range = wk_range;
  cpi->wk_bin_width = wk_bin_width;
  cpi->min_logwk = min_logwk;
  cpi->max_logwk = max_logwk;

#if CONFIG_WK_WKX_NORM_MODE
  log_sum = exp(log_sum / (double)(num_rows * num_cols));

  for (int row = 0; row < num_rows; ++row) {
    for (int col = 0; col < num_cols; ++col) {
      index = row * num_cols + col;
      cpi->wk_rdmult_scaling_factors[index] /= log_sum;
    }
  }
  dump_scaling_maps(cpi->wk_rdmult_scaling_factors, num_rows, num_cols, (char *) cpi->oxcf.tune_cfg.dump_folder, "wk_norm_map.txt");
#endif //WK norm
  frame_id++; // end of compute for this frame id
}

void av1_set_mb_wkx_rdmult_scaling(AV1_COMP *cpi) {
  const CommonModeInfoParams *const mi_params = &cpi->common.mi_params;
  uint8_t *y_buffer = cpi->source->y_buffer;
  const int y_stride = cpi->source->y_stride;

  if (cpi->last_source == NULL) {
    av1_set_mb_wk_rdmult_scaling(cpi);
    return;
  }

  uint8_t *y_buffer_last = cpi->last_source->y_buffer;
  //const int y_stride_last = cpi->last_source->y_stride;
  ...
  int BD = 8;
  double apic = pow(2, BD + 1); //2 **  (BD) # one for wpsnr and one for xpsnr
  apic *= pow((3840 * 2160) / (width * height), 0.5);
  double amin2 = pow(pow(2, BD - 6), 2);
    ...
    for (int col = 0; col < num_cols; ++col) {
      ...
      for (int py = 0; py < 16; py++) {
        ...
        for (int px = 0; px < 16; px++) {
          ...
          energy += fabs(cpi->wk_high_pass_filtered[index2]);
          nb_pixels++;
          pos_t = px + col * 16 + y_stride * (py + row * 16);
          energy += lambda_t * abs(y_buffer[pos_t] - y_buffer_last[pos_t]);
        }
      }
      energy /= 4 * nb_pixels;
      energy = pow(energy, 2);
      ...
      // Do we need to perform some fitting there like in av1_set_mb_ssim_rdmult_scaling ?
#if CONFIG_WK_WKX_INV_MODE
      cpi->wkx_rdmult_scaling_factors[index] = 1/wk;
      log_sum += log(1/wk);
#else
      cpi->wk_rdmult_scaling_factors[index] = wk;
      log_sum += log(wk);
  ...
  dump_scaling_maps(cpi->wkx_rdmult_scaling_factors, num_rows, num_cols, (char *) cpi->oxcf.tune_cfg.dump_folder, "wkx_map.txt");
  ...
  for (int row = 0; row < num_rows; ++row) {
    for (int col = 0; col < num_cols; ++col) {
      index = row * num_cols + col;
      cpi->wkx_rdmult_scaling_factors[index] /= log_sum;
    }
  }
  dump_scaling_maps(cpi->wkx_rdmult_scaling_factors, num_rows, num_cols, (char *) cpi->oxcf.tune_cfg.dump_folder, "wkx_norm_map.txt");
#endif // WK norm
  frame_id++; // end of compute for this frame id
}
#endif // DUMMY
