...
int av1_block_wpsnr(const AV1_COMP *const cpi,
...
#if CONFIG_WKX_MODE
  int enable_wkx_mode = 1;
#else
  int enable_wkx_mode = 0;
#endif
  int wkx_mode = cpi->last_source == NULL ? 0 : enable_wkx_mode;
  ...
  for (row = mi_row / num_mi_w;
    ...
    for (col = mi_col / num_mi_h;
      ...
      if (wkx_mode == 0) {
        geom_mean_of_scale += log(cpi->wk_rdmult_scaling_factors[index]);
      } else {
        geom_mean_of_scale += log(cpi->wkx_rdmult_scaling_factors[index]);
      }
      num_of_mi += 1.0;
    }
  }
  geom_mean_of_scale = exp(geom_mean_of_scale / num_of_mi) * 4;//2.5;
  if (geom_mean_of_scale > 7) geom_mean_of_scale = 7; // to fit in the 8 available segmentation maps
  return (int)(geom_mean_of_scale);
}

double av1_block_logwk(const AV1_COMP *const cpi,
                    const BLOCK_SIZE bsize, const int mi_row,
                    const int mi_col) {
  const AV1_COMMON *const cm = &cpi->common;
  ...
  for (row = mi_row / num_mi_w;
    ...
    for (col = mi_col / num_mi_h;
      ...
      const int index = row * num_cols + col;
      if (wkx_mode == 0) {
        logscore = 3 * log(cpi->wk_rdmult_scaling_factors[index]) / log(2);
        //geom_mean_of_scale += log(logscore); // not practical because some values are negative from log transform
        mean_of_scale += logscore;
      } else {
        logscore = 3 * log(cpi->wkx_rdmult_scaling_factors[index]) / log(2);
        //geom_mean_of_scale += log(logscore);
        mean_of_scale += logscore;
      }
      num_of_mi += 1.0;
    }
  ...
  mean_of_scale = mean_of_scale / num_of_mi;
  //return geom_mean_of_scale;
  return mean_of_scale;
}

void av1_set_rdmult_dummy(const AV1_COMP *const cpi, int *errorperbit,
                          const BLOCK_SIZE bsize, const int mi_row,
  ...
  for (row = mi_row / num_mi_w;
    ...
    for (col = mi_col / num_mi_h;
      ...
      //printf("%f %f %f \n", log(cpi->ssim_rdmult_scaling_factors[index]), cpi->ssim_rdmult_scaling_factors[index], geom_mean_of_scale);

      if (wkx_mode == 0) {
        geom_mean_of_scale += log(cpi->wk_rdmult_scaling_factors[index]);
        mean_of_scale += cpi->wk_rdmult_scaling_factors[index];
      } else {
        geom_mean_of_scale += log(cpi->wkx_rdmult_scaling_factors[index]);
        mean_of_scale += cpi->wkx_rdmult_scaling_factors[index];
      }
      num_of_mi += 1.0;
    }
  }
  ...
  *rdmult = (int)((double)(*rdmult) * mean_of_scale + 0.5);

  // Instead, go for a crazy/naive way to set rdmult
  //if (mi_row < mi_col)
  //    *rdmult = (int)((double)(*rdmult) * 0.1);
  //printf("%d %d %d %f \n", mi_row, mi_col, *rdmult, geom_mean_of_scale);
  //*rdmult = 1000;
...
int av1_block_alpha_beta_aq(const AV1_COMP *const cpi,
                       const BLOCK_SIZE bsize, const int mi_row,
                       const int mi_col) {
  const AV1_COMMON *const cm = &cpi->common;
  ...
  for (row = mi_row / num_mi_w;
    ...
    for (col = mi_col / num_mi_h;
         col < num_cols && col < mi_col / num_mi_h + num_bcols; ++col) {
      const int index = row * num_cols + col;
      mean_of_seg_id += cpi->ab_qps[index];
      num_of_mi += 1.0;
    }
  }
  mean_of_seg_id = mean_of_seg_id / num_of_mi;
  return (int)(mean_of_seg_id);
}

#if CONFIG_A_MODE
int av1_block_alpha_aq(const AV1_COMP *const cpi,
                            const BLOCK_SIZE bsize, const int mi_row,
                            const int mi_col) {
  const AV1_COMMON *const cm = &cpi->common;
  ...
  for (row = mi_row / num_mi_w;
    ...
    for (col = mi_col / num_mi_h;
         col < num_cols && col < mi_col / num_mi_h + num_bcols; ++col) {
      const int index = row * num_cols + col;
      mean_of_seg_id += cpi->factors_a[index];
      num_of_mi += 1.0;
    }
  }

  double bin_size = (cpi->max_factor_a - cpi->min_factor_a) / MAX_SEGMENTS;
  mean_of_seg_id = mean_of_seg_id / num_of_mi;
  mean_of_seg_id = (mean_of_seg_id - cpi->min_factor_a) / bin_size;
...
int av1_block_saliency(const AV1_COMP *const cpi,
                    const BLOCK_SIZE bsize, const int mi_row,
                    const int mi_col) {
  const AV1_COMMON *const cm = &cpi->common;
  ...
  for (row = mi_row / num_mi_w;
    ...
    for (col = mi_col / num_mi_h;
      ...
      for(int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
          const int index = (row * 16 + i) * (num_cols * 16) + (col * 16 + j);
          geom_mean_of_scale += log(cpi->saliency_factors[index]);
          num_of_mi += 1.0;
        }
      }
    }
  ...
  geom_mean_of_scale = exp(geom_mean_of_scale / num_of_mi) * 7;
  if (geom_mean_of_scale > 7) geom_mean_of_scale = 7; // to fit in the 8 available segmentation maps
  return (int)(geom_mean_of_scale);
}

void av1_set_rdmult_saliency(const AV1_COMP *const cpi, int *errorperbit,
                             const BLOCK_SIZE bsize, const int mi_row,
                             const int mi_col, int *const rdmult) {
  const AV1_COMMON *const cm = &cpi->common;
    ...
    for (col = mi_col / num_mi_h;
      ...
      for(int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
          const int index = (row * 16 + i) * (num_cols * 16) + (col * 16 + j);
          geom_mean_of_scale += log(cpi->saliency_factors[index]);
          //printf("%f %f %f \n", log(cpi->saliency_factors[index]), cpi->saliency_factors[index], geom_mean_of_scale);
          num_of_mi += 1.0;
        }
      }
    }
...
#if CONFIG_A_MODE
void av1_set_a_mode_rdmult(const AV1_COMP *const cpi, int *errorperbit,
                         const BLOCK_SIZE bsize, const int mi_row,
                         const int mi_col, int *const rdmult) {
  const AV1_COMMON *const cm = &cpi->common;
  ...
  for (row = mi_row / num_mi_w;
    ...
    for (col = mi_col / num_mi_h;
         col < num_cols && col < mi_col / num_mi_h + num_bcols; ++col) {
      const int index = row * num_cols + col;
      if (cpi->factors_a[index] == 0) { // should not append
        geom_mean_of_scale += 0;
      } else {
        geom_mean_of_scale += log(cpi->factors_a[index]);
      }
      //mean_of_scale += cpi->factors_a[index];
      num_of_mi += 1.0;
    }
  }
  geom_mean_of_scale = exp(geom_mean_of_scale / num_of_mi);
...
#if CONFIG_AB_MODE
void av1_set_rdmult_abmode(const AV1_COMP *const cpi, int *errorperbit,
                             const BLOCK_SIZE bsize, const int mi_row,
                             const int mi_col, int *const rdmult,
                           double *const factor_a, double *const factor_b,
                           double *const spy_tracker) {
  const AV1_COMMON *const cm = &cpi->common;
  ...
  int row, col;
  double num_of_mi = 0.0;
  double geom_mean_of_scale_a = 0.0;
  double geom_mean_of_scale_b = 0.0;
  ...
  for (row = mi_row / num_mi_w;
    ...
    for (col = mi_col / num_mi_h;
      ...
      for(int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
          const int index = (row * 16 + i) * (num_cols * 16) + (col * 16 + j);
          geom_mean_of_scale_a += log(cpi->ab_factors_a[index]);
          geom_mean_of_scale_b += log(cpi->ab_factors_b[index]);
          num_of_mi += 1.0;
        }
      }
    }
  }
  geom_mean_of_scale_a = exp(geom_mean_of_scale_a / num_of_mi);
  geom_mean_of_scale_b = exp(geom_mean_of_scale_b / num_of_mi);

  //*rdmult = (int)((double)(*rdmult) * geom_mean_of_scale + 0.5);
  //*rdmult = AOMMAX(*rdmult, 0);
  *factor_a = geom_mean_of_scale_a;
  *factor_b = geom_mean_of_scale_b;
  *spy_tracker = 31415;
