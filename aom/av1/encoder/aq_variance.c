...
void av1_alpha_beta_vaq_frame_setup(AV1_COMP *cpi) {
  AV1_COMMON *cm = &cpi->common;
  const RefreshFrameInfo *const refresh_frame = &cpi->refresh_frame;
  const int base_qindex = cm->quant_params.base_qindex;
  struct segmentation *seg = &cm->seg;
  ...
  if (frame_is_intra_only(cm) || cm->features.error_resilient_mode ||
    ...
    for (i = 0; i < MAX_SEGMENTS; ++i) {
      ...
      qindex_delta = (int)(cpi->ab_deltaq[i] * 4); // * 4 to match the range of qindex 0-255 compared to 0-63
      // We don't allow qindex 0 in a segment if the base value is not 0.
      // Q index 0 (lossless) implies 4x4 encoding only and in AQ mode a segment
      // Q delta is sometimes applied without going back around the rd loop.
      // This could lead to an illegal combination of partition size and q.
...
#if CONFIG_A_MODE
void av1_alpha_vaq_frame_setup(AV1_COMP *cpi) {
  AV1_COMMON *cm = &cpi->common;
  const RefreshFrameInfo *const refresh_frame = &cpi->refresh_frame;
  const int base_qindex = cm->quant_params.base_qindex;
  struct segmentation *seg = &cm->seg;
  ...
  if (frame_is_intra_only(cm) || cm->features.error_resilient_mode ||
    ...
    double target_rate_ratio;

    int qindex_delta = 0;
    printf("Frame qindex:");
    for (i = 0; i < MAX_SEGMENTS; ++i) {

      target_rate_ratio = cpi->min_factor_a + i * bin_size + bin_size / 2;
      qindex_delta = av1_compute_qdelta_by_rate(
          &cpi->rc, cm->current_frame.frame_type, base_qindex,
          target_rate_ratio, cpi->is_screen_content_type,
          cm->seq_params->bit_depth);
...
void av1_wk_vaq_frame_setup(AV1_COMP *cpi) {
  AV1_COMMON *cm = &cpi->common;
  const RefreshFrameInfo *const refresh_frame = &cpi->refresh_frame;
  const int base_qindex = cm->quant_params.base_qindex;
  struct segmentation *seg = &cm->seg;
  ...
  if (frame_is_intra_only(cm) || cm->features.error_resilient_mode ||
    ...
    for (i = 0; i < MAX_SEGMENTS; ++i) {
      ...
      int qindex_delta = (int)((cpi->min_logwk + (i + 0.5) * cpi->wk_bin_width) * 4); // * 4 to match the range of qindex 0-255 compared to 0-63
      // We don't allow qindex 0 in a segment if the base value is not 0.
      // Q index 0 (lossless) implies 4x4 encoding only and in AQ mode a segment
      // Q delta is sometimes applied without going back around the rd loop.
      // This could lead to an illegal combination of partition size and q.
...
int av1_log_block_var(const AV1_COMP *cpi, MACROBLOCK *x, BLOCK_SIZE bs) {
  ...
  for (i = 0; i < bh; i += 4) { // andreas: iterate over any block size using step of 4x4 block
    for (j = 0; j < bw; j += 4) {
