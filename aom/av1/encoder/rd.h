...
#if CONFIG_AB_MODE
static INLINE int64_t av1_calculate_rd_cost(int mult, double factor_a,
                                            double factor_b, double spy_tracker, int N,
                                            int rate, int64_t dist,
                                            int64_t sse) {
  if (spy_tracker != 31415.0) {
    // the factor_a and factor_b variables are not correctly initialized
    double pause = 0;
  }

  double visibility_th = 0.6; // 0.6
  double mse_vth = (1/factor_b) * log((visibility_th + factor_a) / factor_a);

  double bN = factor_b / ((double) N * 4 * 4); // N is the size of the block in MB_UNITS not in pixel, so need to convert
  double mse = sse / (N * 4 * 4); // just to have the info while debugging
  double mdist = dist / (N * 4 * 4); // just to have the info while debugging

  double exp_data, wab;
  if (mdist > mse_vth) { // high distortion range apply linear function // mse or mdist
    wab = factor_a * factor_b * exp(factor_b * mse_vth); // constant
  } else {
    exp_data = bN * ((double) dist); // sse or dist
    if (exp_data > 700) { // if larger than that the exp will fail
      exp_data = 700;
    } else {
      // no problem
    }
    // when exp_data is large wab is really large, then mult is divided by a large value and egal to 0 when cast to int
    // So in RDCOST the rate R is multiplied by 0 and COST is only dependent on "dist" variable
    // What is the difference between dist and sse variable?
    wab = factor_a * (bN) * exp(exp_data);
    //double temp = ((double) mult) / wab;
  ...
  /*
  // Open the file to append the variables
  FILE* fp = fopen("/home/lambda-ipi/Documents/output.txt", "a");
  if (fp == NULL) {
    printf("Error could not open file for writing ab variables");
  }
  //write the variables to the file
  fprintf(fp, "%d,%.6lf,%.6lf,%.1lf,%d,%d,%ld,%ld,%.6lf,%.6lf,%.6lf\n",
          mult, factor_a, factor_b, spy_tracker, N, rate, dist, sse, bN, wab, mse);
  fclose(fp);
  */
...
static INLINE void av1_rd_cost_update(int mult, double factor_a,
                                      double factor_b, double spy_tracker, int N,
                                      RD_STATS *rd_cost) {
  if (rd_cost->rate < INT_MAX && rd_cost->dist < INT64_MAX &&
      rd_cost->rdcost < INT64_MAX) {
    rd_cost->rdcost = av1_calculate_rd_cost(mult, factor_a, factor_b, spy_tracker,
                                            N, rd_cost->rate, rd_cost->dist,
                                            rd_cost->sse);
  } else {
...
static INLINE void av1_rd_stats_subtraction(int mult, double factor_a,
                                            double factor_b, double spy_tracker, int N,
  ...
  if (left->rate == INT_MAX || right->rate == INT_MAX ||
    ...
    result->rdcost = av1_calculate_rd_cost(mult, factor_a, factor_b, spy_tracker,
                                           N, result->rate, result->dist,
                                           result->sse);
  }
}
#else
