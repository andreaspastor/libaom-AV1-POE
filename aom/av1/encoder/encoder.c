...
AV1_COMP *av1_create_compressor(AV1_PRIMARY *ppi, AV1EncoderConfig *oxcf,
...

  {
    ...
    CHECK_MEM_ERROR(cm, cpi->saliency_factors,
                    aom_calloc(height * width,
                               sizeof(*cpi->saliency_factors)));
#endif
#if CONFIG_TUNE_RDMULT_DUMMY
    CHECK_MEM_ERROR(cm, cpi->wk_rdmult_scaling_factors,
                    aom_calloc(num_rows * num_cols,
                               sizeof(*cpi->wk_rdmult_scaling_factors)));

    CHECK_MEM_ERROR(cm, cpi->wkx_rdmult_scaling_factors,
                    aom_calloc(num_rows * num_cols,
                               sizeof(*cpi->wkx_rdmult_scaling_factors)));

    int height2 = cpi->frame_info.frame_height; // sometime height is already defined and sometime not
    int width2 = cpi->frame_info.frame_width;
    CHECK_MEM_ERROR(cm, cpi->wk_high_pass_filtered,
                    aom_calloc((height2 - 2) * (width2 - 2),
                               sizeof(*cpi->wk_high_pass_filtered)));
#endif
#if CONFIG_A_MODE
    int height = cpi->frame_info.frame_height;
    int width = cpi->frame_info.frame_width;
    CHECK_MEM_ERROR(cm, cpi->factors_a,
                    aom_calloc(num_rows * num_cols,
                               sizeof(*cpi->factors_a)));
#endif
    CHECK_MEM_ERROR(cm, cpi->ab_qps,
                    aom_calloc(num_rows * num_cols,
                               sizeof(*cpi->ab_qps)));
    CHECK_MEM_ERROR(cm, cpi->ab_deltaq,
                    aom_calloc(8,
                               sizeof(*cpi->ab_deltaq)));
#if CONFIG_AB_MODE
    int height = cpi->frame_info.frame_height;
    int width = cpi->frame_info.frame_width;
    CHECK_MEM_ERROR(cm, cpi->ab_factors_a,
                    aom_calloc(height * width,
                               sizeof(*cpi->ab_factors_a)));
    CHECK_MEM_ERROR(cm, cpi->ab_factors_b,
                    aom_calloc(height * width,
                               sizeof(*cpi->ab_factors_b)));
#endif
  }
...
static int encode_with_recode_loop(AV1_COMP *cpi, size_t *size, uint8_t *dest) {
  ...
  do {
    ...
    if (q_cfg->aq_mode == VARIANCE_AQ || q_cfg->aq_mode == WPSNR_AQ || q_cfg->aq_mode == SALIENCY_AQ) {
#if CONFIG_TUNE_RDMULT_DUMMY
      av1_wk_vaq_frame_setup(cpi);
#else
      av1_vaq_frame_setup(cpi);
#endif
    } else if (q_cfg->aq_mode == ALPHA_BETA_AQ) {
      // add frame setup for this mode (setup the different segmentation map)
      av1_alpha_beta_vaq_frame_setup(cpi);
    } else if (q_cfg->aq_mode == ALPHA_AQ) {
#if CONFIG_A_MODE
      // add frame setup for this mode (setup the different segmentation map)
      av1_alpha_vaq_frame_setup(cpi);
#endif
...
static int encode_frame_to_data_rate(AV1_COMP *cpi, size_t *size,
  ...
  if (cpi->oxcf.q_cfg.aq_mode == ALPHA_AQ) {
#if CONFIG_A_MODE
    av1_set_amode(cpi, (char *) cpi->oxcf.tune_cfg.dump_folder, "_alpha.txt");
# endif
  } else if (cpi->oxcf.q_cfg.aq_mode == ALPHA_BETA_AQ) {
#if CONFIG_A_MODE
    // set the alpha beta AQ mode by loading the information to use in the encoding process
    av1_set_alpha_beta_aq(cpi, (char *) cpi->oxcf.tune_cfg.dump_folder, ".txt");
# endif
  } else if (oxcf->tune_cfg.tuning == AOM_TUNE_SSIM || cpi->oxcf.q_cfg.aq_mode == WPSNR_AQ || cpi->oxcf.q_cfg.aq_mode == SALIENCY_AQ) {
#if CONFIG_AB_MODE
    av1_set_abmode_alpha(cpi, (char *) cpi->oxcf.tune_cfg.dump_folder, "_alpha.txt"); // function to load the ab information from outside the codec
    av1_set_abmode_beta(cpi, (char *) cpi->oxcf.tune_cfg.dump_folder, "_beta.txt"); // function to load the ab information from outside the codec
#else
#if CONFIG_SALIENCY_MODE
    av1_set_saliency(cpi, (char *) cpi->oxcf.tune_cfg.dump_folder, "saliency.txt"); // function to load the saliency information from outside the codec
#else
#if CONFIG_TUNE_RDMULT_DUMMY
#if CONFIG_WKX_MODE
    av1_set_mb_wkx_rdmult_scaling(cpi);
#else
    av1_set_mb_wk_rdmult_scaling(cpi);
#endif
#else
#if CONFIG_A_MODE
    //av1_set_amode(cpi, (char *) cpi->oxcf.tune_cfg.dump_folder, "_alpha_linear.txt"); // function to load the ab information from outside the codec
    av1_set_amode(cpi, (char *) cpi->oxcf.tune_cfg.dump_folder, "_alpha.txt");
#else
