...
static aom_codec_err_t set_encoder_config(AV1EncoderConfig *oxcf,
  ...
  tune_cfg->dump_folder = extra_cfg->dump_folder;
...
static aom_codec_err_t ctrl_set_dump_folder(aom_codec_alg_priv_t *ctx,
                                                va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  const char *str = CAST(AV1E_SET_DUMP_FOLDER, args);
  const aom_codec_err_t ret = allocate_and_set_string(
      str, default_extra_cfg.dump_folder, &extra_cfg.dump_folder,
      ctx->ppi->error.detail);
  if (ret != AOM_CODEC_OK) return ret;
  return update_extra_cfg(ctx, &extra_cfg);
}
...
#endif
  check_and_free_string(default_extra_cfg.dump_folder,
                        &extra_cfg->dump_folder);
...
#endif
  else if (arg_match_helper(&arg, &g_av1_codec_arg_defs.dump_folder, argv,
                            err_string)) {
    err = allocate_and_set_string(value, default_extra_cfg.dump_folder,
                                  &extra_cfg.dump_folder, err_string);
  }
...
static aom_codec_ctrl_fn_map_t encoder_ctrl_maps[] = {
  ...
  { AV1E_SET_DUMP_FOLDER, ctrl_set_dump_folder },
