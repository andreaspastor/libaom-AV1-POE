...
#endif
  &g_av1_codec_arg_defs.dump_folder,
...
#endif
    } else if (arg_match(&arg, &g_av1_codec_arg_defs.dump_folder, argi)) {
      config->dump_folder = arg.val;
...
#endif
  if (stream->config.dump_folder) {
    AOM_CODEC_CONTROL_TYPECHECKED(&stream->encoder, AV1E_SET_DUMP_FOLDER,
                                  stream->config.dump_folder);
  }
